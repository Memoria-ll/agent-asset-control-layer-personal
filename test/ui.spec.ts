...
test('C33: Chromium UI assigns existing and new Roles from Workflow editor, runs a Workflow, records and reviews Journal', async ({ page }) => {
  ...
  await dialog.getByLabel('Model').nth(0).selectOption({ label: '実装Model / provider/implementer（Global）' });
  ...
  await dialog.getByLabel('Model').nth(1).selectOption({ label: '実装Model / provider/implementer（Global）' });
