...
test('Journal Skills are protected, journal is not a direct entry, and recording can be disabled', async t => {
  ...
  assert.match(journal.description, /^タスク完了時に、実際に使ったTool・Skill・Rule/);
  assert.ok(!journal.body.includes('明確な設計・実装タスクの区切りで、'));
  assert.match(journal.body, /tools: \[\]/);
  assert.match(journal.body, /skills: \[\]/);
  assert.match(journal.body, /rules: \[\]/);
  assert.match(journal.body, /コードの変更内容や差分ではなく/);
  assert.match(review.body, /新しいWorkflow・Skill・Role・Rule・Model・紐づけの候補/);
  assert.match(review.body, /未観測/);
  assert.match(review.body, /レビューで扱ったJournalの一覧/);
  assert.match(review.body, /共有Assetを参照する他Projectへの影響/);
  const template = (await f.call<{ template: string }>('journal.template')).template;
  assert.match(template, /tools: \[\][\s\S]*skills: \[\][\s\S]*rules: \[\]\n/);
