...
export function installJournalSkills(core: Core) {
  ...
  const existing = core.store.list<Asset>('asset');
  const changes: Change[] = definitions.filter(d => !existing.some(a => a.metadata.aaclUtility === d.key)).map(d => ({ type: 'asset.save', asset: assetSchema.parse({ kind: 'skill', name: d.name, description: d.description, body: d.body, useCase: true, metadata: { aaclUtility: d.key } }) }));
