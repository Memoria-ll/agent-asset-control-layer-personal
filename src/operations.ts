...
type RuntimeSyncSelector = boolean | ((result: unknown) => Iterable<string>);
...
export class Operations {
  ...
  constructor(core: Core) {
    ...
    const changedAssetIds: RuntimeSyncSelector = result => {
      const ids = new Set<string>();
      for (const entity of (result as Partial<ReturnType<Core['applyChanges']>>).entities ?? []) {
        if ('kind' in entity) ids.add(entity.id);
        if ('sourceId' in entity) {
          ...
          if (entity.revision > 1) bindings.push(store.revision<Binding>(entity.id, entity.revision - 1));
          for (const binding of bindings) { ids.add(binding.sourceId); ids.add(binding.targetId); }
        }
      }
      return ids;
    };
