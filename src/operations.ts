...
type RuntimeSyncSelector = boolean | ((fields: Record<string, unknown>, result: unknown) => Iterable<string> | undefined);
const assetKinds = new Set(['workflow', 'skill', 'role', 'rule', 'model']);
const recordsFromResult = (result: unknown) => {
  if (!result || typeof result !== 'object' || Array.isArray(result)) return [] as Record<string, unknown>[];
  const entities = (result as { entities?: unknown }).entities;
  return Array.isArray(entities) ? entities.filter((entity): entity is Record<string, unknown> => Boolean(entity) && typeof entity === 'object' && !Array.isArray(entity)) : [];
};
const assetIdsFromResult = (result: unknown) => new Set(recordsFromResult(result).flatMap(entity => typeof entity.id === 'string' && typeof entity.kind === 'string' && assetKinds.has(entity.kind) ? [entity.id] : []));
const assetIdsFromResultAndRelations = (result: unknown) => {
  const ids = assetIdsFromResult(result);
  for (const entity of recordsFromResult(result)) for (const key of ['sourceId', 'targetId']) if (typeof entity[key] === 'string') ids.add(entity[key]);
  return ids;
};
const resultAssetIds: RuntimeSyncSelector = (_fields, result) => assetIdsFromResult(result);
const resultAssetIdsAndRelations: RuntimeSyncSelector = (_fields, result) => assetIdsFromResultAndRelations(result);
const assetIdsFromChanges = (fields: Record<string, unknown>, result: unknown) => {
  ...
  for (const change of changes) {
    ...
    if (typeof item.id === 'string' && ['asset.save', 'asset.update', 'asset.create', 'asset.delete'].includes(String(item.type))) ids.add(item.id);
    const binding = item.binding;
    if (binding && typeof binding === 'object' && !Array.isArray(binding)) {
      for (const key of ['sourceId', 'targetId']) if (typeof (binding as Record<string, unknown>)[key] === 'string') ids.add((binding as Record<string, unknown>)[key] as string);
    }
  }
  return ids;
};
const bindingAssetIds = (fields: Record<string, unknown>, result: unknown) => {
...
export class Operations {
  ...
  constructor(core: Core) {
    ...
    const write = <T extends z.ZodRawShape>(name: string, description: string, shape: T, fn: (input: z.output<z.ZodObject<T>>) => unknown, sync: RuntimeSyncSelector = false) => this.register(name, description, z.object(shape).strict(), fn, true, sync);
    ...
    const assetView = (asset: Asset, includeBody: boolean, fields: string[]) => {
      const result = { ...asset } as Record<string, unknown>;
      if (!includeBody && !fields.includes('body')) delete result.body;
      if (!includeBody && !fields.includes('supportingFiles')) delete result.supportingFiles;
      return result;
    };
    ...
    read('proposal.list', '改善提案・ユーザー判断・適用結果を一覧', { includeChanges: z.boolean().default(false) }, p => ({ proposals: store.list<Proposal>('proposal').map(proposal => {
    ...
    write('settings.save', 'Runの非活動timeoutとJournal記録の有効・無効を設定', { timeoutHours: z.number().positive().max(8760).optional(), journalEnabled: z.boolean().optional() }, p => {
      if (p.timeoutHours === undefined && p.journalEnabled === undefined) throw new Error('変更する設定を1件以上指定してください。');
  ...
  register<T extends z.ZodObject>(name: string, description: string, schema: T, handler: (input: z.output<T>) => unknown, write = false, sync: RuntimeSyncSelector = false, mcpVisible = true) {
    ...
    this.entries.set(name, { description, schema: inputSchema, write, mcpVisible, execute: async raw => {
      ...
      return { ...(result as object), runtimeSync: this.runtime.sync(assetIds) };
    } });
  }
