...
type PowerShellResult = { status: number | null; stdout: string; stderr: string; error?: Error };
type PowerShellExecutor = (script: string) => PowerShellResult;

function runPowerShell(script: string): PowerShellResult {
...
function powerShellString(value: string) { return `'${value.replaceAll("'", "''")}'`; }
function checkedPowerShell(body: string) {
...
export class WindowsAutostart {
  ...
  constructor(directory: string, distroName = process.env.WSL_DISTRO_NAME, execute: PowerShellExecutor = runPowerShell) {
  ...
  private invoke(script: string, expected: number[] = [0]) {
  ...
  enable() {
    if (!this.distroName) throw new Error('Windows自動起動はWSL内から設定してください。');
    ...
    const script = checkedPowerShell(
      `$action = New-ScheduledTaskAction -Execute (Join-Path $env:WINDIR 'System32\\wsl.exe') -Argument ${args}\n` +
