import type { Asset, AssetDeletionPreview, Binding, Change, ChangeSet, Common, Decision, Delivery, Diagnostic, History, Insight, Journal, Project, Proposal, Provenance, Run, RunEvent, RuntimeTarget, Snapshot } from '../src/schema.ts';
...
const select = (label: string, key: string, options: string) => `<label>${esc(label)}<select name="${key}">${options}</select></label>`;
...
function modal(title: string, html: string) { dialog.innerHTML = `<div class="dialog-head"><h2>${esc(title)}</h2>${button('close', '×', 'icon-button')}</div>${html}`; if (!dialog.open) dialog.showModal(); }
...
async function render() {
  ...
  if (page === 'runs') {
    ...
    const data = await api<{ journals: Journal[]; insights: Insight[] }>(page === 'review' ? 'review.pending' : 'journal.list');
    const proposals = page === 'review' ? await api<{ proposals: Proposal[]; decisions: Decision[]; changeSets: ChangeSet[] }>('proposal.list') : { proposals: [], decisions: [], changeSets: [] };
...
function stageRow(s: { id: string; name: string; completion_condition: string; description?: string; taskType?: string }, index: number, roleId = '', transitions: Asset['transitions'] = [], stages: { id: string; name: string }[] = []) {
...
function bindingEditor(sourceId: string, stageId?: string, existing?: Binding) {
  ...
  modal(existing ? '紐づけを付け替え' : '資産を紐づける', `<form data-form="binding" data-id="${existing?.id ?? ''}" data-source="${sourceId}" data-stage="${esc(stageId ?? '')}" class="form-stack"><p>${esc(a.name)}${stageId ? ` / ${esc(a.stages.find(s => s.id === stageId)?.name)}` : ''} → 参照先</p>${select('参照する資産', 'targetId', targets.map(t => opt(t.id, `${kinds[t.kind]} / ${t.name}`, existing?.targetId)).join(''))}${select('使い方', 'purpose', opt('reference', '資産の参照', existing?.purpose) + (a.kind === 'workflow' ? opt(stageId ? 'stage-role' : 'entry-role', stageId ? '工程の担当Role' : '入口のRole', existing?.purpose) : ''))}<p class="hint">管理先: ${esc(labelScope(selectedScope))}</p>${formEnd('紐づけを保存')}</form>`);
}
...
async function action(value: string, target: HTMLElement) {
  ...
  if (key === 'stage-add') { const root = dialog.querySelector('#stage-rows')!; root.insertAdjacentHTML('beforeend', stageRow({ id: crypto.randomUUID(), name: '', completion_condition: '' }, root.children.length, '', [], readStages(dialog))); updateTransitionTargets(dialog); return; }
  if (key === 'role-create') {
