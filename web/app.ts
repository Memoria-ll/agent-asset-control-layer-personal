import type { Asset, AssetDeletionPreview, AssetSummary, Binding, Change, ChangeSet, Common, Decision, Delivery, Diagnostic, History, Insight, Journal, Project, Proposal, Provenance, ReviewItem, Run, RunEvent, RuntimeTarget, Snapshot } from '../src/schema.ts';
import { diagnosticAsset, diagnosticAssetId, relatedWorkflows, stageModelBindingChanges, stageRoleBindingChanges, workflowDiagram } from './view-model.ts';
...
let assets: AssetSummary[] = [], projects: Project[] = [], bindings: Binding[] = [];
const assetDetails = new Map<string, Asset>();
let assetDetailRequest: { id: string; token: number } | undefined;
let assetDetailRequestToken = 0;
let assetDetailError: { id: string; message: string } | undefined;
...
let theme: Theme = localStorage.getItem('aacl-theme') === 'dark' ? 'dark' : 'light';
...
const journalSkillKey = (asset: Pick<Asset, 'kind' | 'metadata'>) => asset.kind === 'skill' && (asset.metadata.aaclUtility === 'journal' || asset.metadata.aaclUtility === 'journal-review') ? asset.metadata.aaclUtility : undefined;
...
function syncThemeControl() {
  ...
  control.querySelector<HTMLElement>('.theme-toggle-thumb')!.textContent = dark ? '☾' : '☀';
  control.querySelector<HTMLElement>('.theme-toggle-value')!.textContent = localizeHtml(dark ? 'ダーク' : 'ライト', language);
}
...
function isFullAsset(asset: AssetSummary | undefined): asset is Asset {
...
function cacheAssetEntities(result: unknown) {
  if (!result || typeof result !== 'object' || Array.isArray(result)) return;
  const entities = (result as { entities?: unknown }).entities;
  if (!Array.isArray(entities)) return;
  for (const entity of entities) if (entity && typeof entity === 'object' && !Array.isArray(entity) && isFullAsset(entity as AssetSummary)) cacheAsset(entity as Asset);
}
async function ensureAssetDetails(id: string) {
  const summary = assets.find(asset => asset.id === id), cached = assetDetails.get(id);
  if (cached && (!summary || cached.revision === summary.revision)) return cacheAsset(cached);
  const result = await api<{ asset: Asset }>('asset.get', { assetId: id });
  return cacheAsset(result.asset);
}
async function loadAssetDetail(id: string, token: number) {
  try {
    const result = await api<{ asset: Asset }>('asset.get', { assetId: id });
    if (!assetDetailRequest || assetDetailRequest.id !== id || assetDetailRequest.token !== token || route()[0] !== 'assets' || route()[1] !== id) return;
    assetDetailError = undefined;
    assetDetailRequest = undefined;
    cacheAsset(result.asset);
    renderAssets();
  } catch (error) {
    if (!assetDetailRequest || assetDetailRequest.id !== id || assetDetailRequest.token !== token) return;
    assetDetailRequest = undefined;
...
function scopeSummary(assetScope?: string) {
  const scopeName = (scope: string) => esc(scope === 'global' ? 'Global' : `Project / ${labelScope(scope)}`);
...
function implicitInvocationField(a?: Asset) {
...
function assetDrawer(a: AssetSummary) {
...
function diagnosticAssetCard(evidence: unknown, catalog: AssetSummary[]) {
...
function modelChoiceSummary(model: AssetSummary) {
  return (model.choices ?? []).map(choice => `${choice.name}: ${choice.options.join(' / ')}`).join(' · ');
}
function selectedChoiceSummary(model: AssetSummary | undefined, selected: Record<string, string> = {}, ids: Record<string, string> = {}) {
  if (!model) return '';
...
function stageModelSelectionIds(workflowId: string, stageId: string) {
  return bindings.find(b => b.sourceId === workflowId && b.stageId === stageId && b.purpose === 'stage-model')?.selectedChoiceIds ?? {};
}
function missingIdOption(id: string) {
  return `<option value="" data-missing-id="${esc(id)}" selected>IDが見つかりません（再選択）</option>`;
}
function modelChoiceFields(modelId: string, selected: Record<string, string> = {}, selectedIds: Record<string, string> = {}) {
...
function readModelSelections(row: Element) {
  return Object.fromEntries([...row.querySelectorAll<HTMLSelectElement>('[name=modelChoice]')].map(input => {
    const missingId = input.selectedOptions[0]?.dataset.missingId;
    if (missingId) throw new Error(`Modelの選択肢ID「${missingId}」が見つかりません。選択し直してください。`);
    return [input.dataset.choiceName ?? '', input.value];
  }).filter(([name, value]) => name && value));
}
function readModelSelectionIds(row: Element) {
  return Object.fromEntries([...row.querySelectorAll<HTMLSelectElement>('[name=modelChoice]')].map(input => {
    const missingId = input.selectedOptions[0]?.dataset.missingId;
    if (missingId) throw new Error(`Modelの選択肢ID「${missingId}」が見つかりません。選択し直してください。`);
...
function choiceConditionSummary(model: AssetSummary | undefined, conditions: Record<string, string>[] = [], ids: Record<string, string>[] = []) {
...
function modelChoiceConditionRow(model: AssetSummary, condition: Record<string, string> = {}, index = 0, ids: Record<string, string> = {}) {
...
function modelChoiceConditionFields(modelId: string, conditions: Record<string, string>[] = [], ids: Record<string, string>[] = []) {
  const model = assets.find(a => a.id === modelId && a.kind === 'model');
  if (!model?.choices?.length) return '';
...
function assetCheckboxPicker(label: string, key: string, items: AssetSummary[], selected: string[]) {
...
function readModelChoiceConditionData(form: Element) {
  const rows = [...form.querySelectorAll<HTMLElement>('.choice-condition-row')].map(row => {
    const values: Record<string, string> = {}, ids: Record<string, string> = {};
    for (const input of row.querySelectorAll<HTMLSelectElement>('[name=choiceCondition]')) {
      const missingId = input.selectedOptions[0]?.dataset.missingId;
      if (missingId) throw new Error(`Modelの選択肢ID「${missingId}」が見つかりません。条件を選び直してください。`);
      const name = input.dataset.conditionName ?? '', value = input.value, id = input.selectedOptions[0]?.dataset.optionId;
      if (name && value && id) { values[name] = value; ids[name] = id; }
    }
    return { values, ids };
...
function stageRow(s: { id: string; name: string; additionalInstructions?: string }, index: number, roleId = '', modelId = '', selectedChoices: Record<string, string> = {}, selectedChoiceIds: Record<string, string> = {}, transitions: Asset['transitions'] = [], stages: { id: string; name: string }[] = []) {
...
function modelChoiceEditorRow(choice: { name?: string; options?: string[]; optionIds?: string[] } = {}) {
...
function readModelChoices(form: Element) {
  return [...form.querySelectorAll<HTMLElement>('.model-choice-editor-row')].map(row => ({
    ...
    optionIds: [...row.querySelectorAll<HTMLInputElement>('[name=choiceOptionId]')].map(input => input.value),
  }));
}
...
async function action(value: string, target: HTMLElement) {
  ...
  if (key === 'asset-detail-retry') { assetDetailError = undefined; assetDetailRequest = undefined; renderAssets(); return; }
  ...
  if (key === 'asset-edit') { if (!a) throw new Error('Assetを再取得してください。'); assetEditor(await ensureAssetDetails(a.id)); return; }
  if (key === 'asset-delete') {
  ...
  if (key === 'choice-option-add') { target.closest<HTMLElement>('.model-choice-editor-row')!.querySelector('.model-option-rows')!.insertAdjacentHTML('beforeend', localizeHtml(`<div class="row model-option-row"><input type="hidden" name="choiceOptionId" value="${crypto.randomUUID()}">${field('選択値', 'choiceOption')}${button('choice-option-remove', '×', 'small ghost')}</div>`, language)); return; }
  ...
  if (key === 'implicit-invocation') { await api('asset.update', { id, expectedRevision: a!.revision, asset: { implicitInvocation: !a!.implicitInvocation }, provenance: provenance('自動発火設定を変更') }, true); await refresh(); return; }
  ...
  if (key === 'project-new') { modal('Projectを登録', `<form data-form="project" class="form-stack">${field('Project名', 'name')}${field('Project root（絶対パス）', 'root')}<p class="hint">登録時点のGlobalの紐づけをコピーします。その後のGlobalの変更は自動同期されません。</p>${formEnd('Projectを登録')}</form>`); return; }
...
async function submit(form: HTMLFormElement) {
  ...
  if (key === 'asset-delete') {
    ...
    const old = form.dataset.id ? await ensureAssetDetails(form.dataset.id) : undefined, kind = form.dataset.kind as Asset['kind'];
    const stages = readStages(form);
    ...
    if (kind === 'workflow') {
      ...
      const assignments = [...form.querySelectorAll<HTMLElement>('.stage-editor')].map(row => {
        ...
        if (!panel.hidden) {
          ...
          newRoleChanges.push({ type: 'asset.create', id: roleId, asset: { kind: 'role', name: roleName, description, explanation: '', body: '', responsibilities: (row.querySelector('[name=newRoleResponsibilities]') as HTMLTextAreaElement).value, scope: 'global', useCase: false, implicitInvocation: false, modelName: '', invocationMethod: '', choices: [], metadata: {}, supportingFiles: {}, stages: [], transitions: [], entryStage: '' } });
          return { stageId, roleId, modelId: (row.querySelector('[name=stageModel]') as HTMLSelectElement).value, selectedChoices: readModelSelections(row), selectedChoiceIds: readModelSelectionIds(row) };
        }
        return { stageId, roleId: (row.querySelector('[name=stageRole]') as HTMLSelectElement).value, modelId: (row.querySelector('[name=stageModel]') as HTMLSelectElement).value, selectedChoices: readModelSelections(row), selectedChoiceIds: readModelSelectionIds(row) };
      });
  ...
  dialog.close(); notify(key === 'asset-delete' ? 'Assetを削除し、参照を解除しました。' : key === 'run' ? 'Runを開始しました。' : '保存しました。'); void refresh();
}
...
document.addEventListener('click', event => {
  const themeToggle = (event.target as HTMLElement).closest<HTMLButtonElement>('#theme-toggle');
  if (themeToggle) {
...
document.addEventListener('input', event => {
  ...
  if (input.matches('[data-asset-picker-search]')) {
    const picker = input.closest<HTMLElement>('[data-asset-picker]');
    if (!picker) return;
    const query = input.value.trim().toLocaleLowerCase();
    const selectElement = picker.querySelector<HTMLSelectElement>('select');
    const items = selectElement ? [...selectElement.options] : [...picker.querySelectorAll<HTMLElement>('[data-asset-item]')];
    let visible = 0;
    for (const item of items) {
      const selected = selectElement ? (item as HTMLOptionElement).value === selectElement.value : Boolean(item.querySelector<HTMLInputElement>('input')?.checked);
      const matches = !query || selected || item.textContent?.toLocaleLowerCase().includes(query);
      item.hidden = !matches;
      if (matches) visible++;
    }
    const emptyMessage = picker.querySelector<HTMLElement>('.asset-picker-empty');
...
let scrollingElement: Element | undefined;
...
document.addEventListener('scroll', event => {
  ...
  if (scrollingElement !== target) { endScrolling(); scrollingElement = target; target.classList.add('is-scrolling'); }
  clearTimeout(scrollIdleTimer);
  scrollIdleTimer = window.setTimeout(endScrolling, 150);
}, { capture: true, passive: true });
document.addEventListener('mousemove', () => { if (scrollingElement) endScrolling(); }, { capture: true, passive: true });
