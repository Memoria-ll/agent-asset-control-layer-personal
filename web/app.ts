...
function stageRow(s: { id: string; name: string; completion_condition: string; additionalInstructions?: string }, index: number, roleId = '', modelId = '', selectedChoices: Record<string, string> = {}, transitions: Asset['transitions'] = [], stages: { id: string; name: string }[] = []) {
...
async function action(value: string, target: HTMLElement) {
  ...
  if (key === 'choice-add') { dialog.querySelector('#model-choice-rows')!.insertAdjacentHTML('beforeend', modelChoiceEditorRow()); return; }
  if (key === 'choice-remove') { target.closest('.model-choice-editor-row')?.remove(); return; }
