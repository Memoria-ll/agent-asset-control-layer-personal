...
function autoInvocationPanel(a: Asset) {
...
async function action(value: string, target: HTMLElement) {
  ...
  if (key === 'autoinvocation') { await api('skill.autoinvocation', { assetId: id, enabled: !a!.autoInvocation, provenance: provenance('自動発火設定を変更') }, true); await refresh(); return; }
