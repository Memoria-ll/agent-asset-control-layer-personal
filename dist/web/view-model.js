    ...
    const walk = (target, via, visited, attachment) => {
        ...
        for (const b of bindings.filter(b => b.active && b.targetId === target)) {
            ...
            if (source.kind === 'workflow')
                result.push({ workflow: source, stageId: b.stageId, via, binding: attachment ?? b });
            else
                walk(source.id, [source.name, ...via], next, attachment ?? b);
        }
    };
    walk(assetId, [], new Set());
    return result;
}
export function stageRoleBindingChanges(workflowId, scope, assignments, bindings) {
    ...
    for (const binding of current) {
        ...
        if (!roleId)
            ...
            changes.push({ type: 'binding.save', id: binding.id, binding: { scope, sourceId: workflowId, stageId, targetId: roleId, purpose: 'stage-role' } });
