import { relatedWorkflows, stageRoleBindingChanges, workflowDiagram } from './view-model.js';
const app = document.querySelector('#app');
const dialog = document.querySelector('#dialog');
const kinds = { workflow: 'Workflow', skill: 'Skill', role: 'Role', rule: 'Rule' };
const symbols = { workflow: '◇', skill: '✧', role: '◉', rule: '≡' };
...
async function api(operation, input = {}, write = false) {
    ...
    if (data.runtimeSync?.some((r) => !r.ok))
...
async function refresh() {
    ...
    loading = true;
    try {
        const [a, p, b] = await Promise.all([api('asset.list'), api('project.list'), api('binding.list', { scope: selectedScope })]);
        assets = a.assets;
        projects = p.projects;
        bindings = b.bindings;
        await render();
...
async function render() {
    ...
    if (page === 'runs') {
        ...
        const data = await api(page === 'review' ? 'review.pending' : 'journal.list');
        const proposals = page === 'review' ? await api('proposal.list') : { proposals: [], decisions: [], changeSets: [] };
        ...
        }).join('')}</div><div class="spacer"></div>` : '') + `<div class="stack">${journals.length ? journals.map(j => `<article class="glass card"><div class="row spread"><div><span class="mono">${date(j.createdAt)}</span><h2>${esc(j.task || (j.runId ? 'Runの振り返り' : 'Journal'))}</h2></div>${badge(j.runId ? 'Runに関連' : 'Taskに関連')}</div>${data.insights.filter(i => i.journalId === j.id).map(i => `<div class="insight"><div class="row spread"><strong>${esc(i.heading)}</strong>${status(i.status)}</div><p>${esc(i.body)}</p><div class="insight-actions">${['pending', 'processed', 'rejected'].filter(s => s !== i.status).map(s => button(`insight:${i.id}:${s}`, states[s])).join('')}</div></div>`).join('')}${details('Journal原文', j.raw)}${j.runId ? `<p class="hint"><a href="#runs/${j.runId}">関連Run・Snapshot・実行記録を見る →</a></p>` : ''}</article>`).join('') : `<div class="glass">${empty(page === 'review' ? 'レビュー待ちの気づきはありません' : '気づきを、次の改善へ', '書き残したい発見や摩擦があるときに、Journalを記録してください。', button('journal-new', 'Journalを記録'))}</div>`}</div>`);
    }
...
function stageRow(s, index, roleId = '', transitions = [], stages = []) {
...
function readStages(form) { return [...form.querySelectorAll('.stage-editor')].map(row => ({ id: row.dataset.id, name: row.querySelector('[name=stageName]').value, completion_condition: row.querySelector('[name=completion]').value, additionalInstructions: row.querySelector('[name=additionalInstructions]').value, description: row.querySelector('[name=stageDescription]').value, taskType: row.querySelector('[name=stageType]').value })); }
...
function bindingEditor(sourceId, stageId, existing) {
    ...
    const allowed = { workflow: ['role', 'skill', 'rule'], role: ['skill', 'rule'], skill: ['skill'], rule: [] };
    const targets = assets.filter(v => v.id !== sourceId && (v.scope === 'global' || v.scope === selectedScope) && allowed[a.kind].includes(v.kind));
    modal(existing ? '紐づけを付け替え' : '資産を紐づける', `<form data-form="binding" data-id="${existing?.id ?? ''}" data-source="${sourceId}" data-stage="${esc(stageId ?? '')}" class="form-stack"><p>${esc(a.name)}${stageId ? ` / ${esc(a.stages.find(s => s.id === stageId)?.name)}` : ''} → 参照先</p>${select('参照する資産', 'targetId', targets.map(t => opt(t.id, `${kinds[t.kind]} / ${t.name}`, existing?.targetId)).join(''))}${existing?.purpose === 'stage-role' ? `<input type="hidden" name="purpose" value="stage-role"><p class="hint">担当RoleはStageごとに必須です。</p>` : select('使い方', 'purpose', opt('reference', '資産の参照', existing?.purpose) + (a.kind === 'workflow' ? opt(stageId ? 'stage-role' : 'entry-role', stageId ? '工程の担当Role' : '入口のRole', existing?.purpose) : ''))}<p class="hint">管理先: ${esc(labelScope(selectedScope))}</p>${formEnd('紐づけを保存')}</form>`);
}
...
async function action(value, target) {
    ...
    if (key === 'binding-remove') {
        await api('binding.remove', { id, provenance: provenance('紐づけを解除') }, true);
        notify('紐づけを解除しました。');
        await refresh();
        return;
    }
    ...
    if (key === 'asset-restore') {
        await api('asset.restore', { assetId: id, revision: Number(extra) }, true);
        dialog.close();
...
async function submit(form) {
    ...
    if (key === 'asset-delete') {
        ...
        if (kind === 'workflow') {
            ...
            const assignments = [...form.querySelectorAll('.stage-editor')].map(row => {
                ...
                if (!panel.hidden) {
                    ...
                    newRoleChanges.push({ type: 'asset.create', id: roleId, asset: { kind: 'role', name: roleName, description, body: '', responsibilities: row.querySelector('[name=newRoleResponsibilities]').value, scope: 'global', useCase: false, taskType: '', metadata: {}, supportingFiles: {}, stages: [], transitions: [], entryStage: '' } });
                    return { stageId, roleId };
                }
                return { stageId, roleId: row.querySelector('[name=stageRole]').value };
            });
            const roleChanges = stageRoleBindingChanges(workflowId, selectedScope, assignments, bindings);
            const workflowChange = old ? { type: 'asset.save', id: workflowId, asset } : { type: 'asset.create', id: workflowId, asset };
            const changes = [
            ...
            const result = await api('asset.save', { id: old?.id, asset, provenance: provenance(old ? '資産を編集' : '資産を作成') }, true);
            location.hash = `assets/${result.entities[0].id}`;
        }
    }
    else if (key === 'binding')
        await api('binding.save', { id: form.dataset.id || undefined, binding: { scope: selectedScope, sourceId: form.dataset.source, stageId: form.dataset.stage || undefined, targetId: get('targetId'), purpose: get('purpose') }, provenance: provenance('資産の紐づけを編集') }, true);
    else if (key === 'run') {
        ...
        await api('common.save', { projectId: selectedScope, ruleIds: data.getAll('ruleIds'), provenance: provenance('Project共通Ruleを編集') }, true);
    else if (key === 'runtime')
