...
function journalSkillPanel(a) {
    ...
    if (utility === 'journal')
        return `<section class="section"><h3>Journal記録</h3><p class="hint">このSkillは直接起動せず、設定・接続のJournal記録設定でON/OFFを切り替えます。</p></section><section class="section"><h3>Runtimeのdescription ${info('Runtimeのdescription', 'Runtime YAMLへ出力されるSkillの説明です。')}</h3><p class="body-panel prose">${esc(a.description)}</p></section>`;
    if (utility === 'journal-review')
        return `<div class="row spread section"><div><h3>直接起動</h3><p class="hint">Journal Reviewはユーザーが明示的に開始する直接起動Skillです。</p></div><button type="button" role="switch" aria-checked="${a.useCase}" data-action="usecase:${a.id}" class="${a.useCase ? 'primary' : ''}">${a.useCase ? '有効' : '無効'}</button></div><section class="section"><h3>Runtimeのdescription ${info('Runtimeのdescription', 'Runtime YAMLへ出力されるSkillの説明です。')}</h3><p class="body-panel prose">${esc(a.description)}</p></section>`;
        ...
        if (kind === 'workflow') {
            ...
            const assignments = [...form.querySelectorAll('.stage-editor')].map(row => {
                ...
                if (!panel.hidden) {
                    ...
                    newRoleChanges.push({ type: 'asset.create', id: roleId, asset: { kind: 'role', name: roleName, description, explanation: '', body: '', responsibilities: row.querySelector('[name=newRoleResponsibilities]').value, scope: 'global', useCase: false, modelName: '', invocationMethod: '', choices: [], metadata: {}, supportingFiles: {}, stages: [], transitions: [], entryStage: '' } });
