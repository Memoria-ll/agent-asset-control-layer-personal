...
const states = { active: '進行中', completed: '完了', cancelled: '中止', failed: '終了・失敗', pending: '保留', processed: '処理済み', rejected: '却下', approved: '承認', deferred: '保留' };
...
const esc = (value) => htmlText(value).replace(/[^\x00-\x7F]/gu, c => `&#x${c.codePointAt(0).toString(16)};`);
const date = (value) => new Date(value).toLocaleString(language === 'en' ? 'en-US' : 'ja-JP', { dateStyle: 'short', timeStyle: 'short' });
const name = (id) => assets.find(a => a.id === id)?.name ?? id;
const journalSkillKey = (asset) => asset.kind === 'skill' && (asset.metadata.aaclUtility === 'journal' || asset.metadata.aaclUtility === 'journal-review') ? asset.metadata.aaclUtility : undefined;
const labelScope = (id) => id === 'global' ? 'Global' : projects.find(p => p.id === id)?.name ?? id;
const icon = (path) => `<svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="${path}"/></svg>`;
const badge = (text, color = '') => `<span class="badge ${color}">${htmlText(text)}</span>`;
const status = (s) => badge(states[s] ?? s, ['completed', 'approved', 'processed', 'active'].includes(s) ? 'green' : ['failed', 'rejected'].includes(s) ? 'red' : 'amber');
const button = (action, label, cls = '') => `<button type="button" class="${cls}" data-action="${esc(action)}">${htmlText(label)}</button>`;
const empty = (title, description, action = '', compact = false) => `<div class="empty ${compact ? 'small' : ''}"><div class="empty-symbol" aria-hidden="true">◇</div><h2>${htmlText(title)}</h2><p>${htmlText(description)}</p>${action}</div>`;
const details = (title, data) => `<details><summary>${htmlText(title)}</summary><pre>${esc(JSON.stringify(data, null, 2))}</pre></details>`;
const opt = (value, label, selected) => `<option value="${esc(value)}"${value === selected ? ' selected' : ''}>${htmlText(label)}</option>`;
const info = (_label, explanation) => `<button type="button" class="info-button" aria-label="説明を表示" data-tooltip="${htmlText(explanation)}">i</button>`;
...
let recordObserver;
let journalListState = { journals: [], insights: [], nextCursor: null };
let reviewListState = { reviewItems: [], journals: [], insights: [], nextCursor: null };
let historyListState = { histories: [], changeSets: [], provenance: [], nextCursor: null };
function notify(message) { const t = document.querySelector('#toast'); t.textContent = message; t.classList.add('visible'); clearTimeout(toastTimer); toastTimer = setTimeout(() => t.classList.remove('visible'), 5000); }
async function api(operation, input = {}, write = false, signal) {
    ...
    if (data.runtimeSync?.failureCount > 0)
...
async function refresh() {
    ...
    const read = (operation, input = {}) => api(operation, input, false, controller.signal);
    loading = true;
    resetRecordState();
    try {
        const [a, p, b, j] = await Promise.all([read('asset.list'), read('project.list'), read('binding.list', { scope: selectedScope }), read('journal.list', { ...(selectedScope !== 'global' ? { projectId: selectedScope } : {}) })]);
        ...
        if (controller.signal.aborted)
            return;
        shell(`<div class="glass error-panel"><h2>読み込めませんでした</h2><p>${esc(error.message)}</p>${button('refresh', '再読み込み')}</div>`);
    }
    finally {
...
function scopeSummary(assetScope) {
    const scopeName = (scope) => esc(scope === 'global' ? 'Global' : `Project / ${labelScope(scope)}`);
    return `<aside class="scope-summary" aria-label="資産と紐づけの管理先"><dl>${assetScope ? `<div><dt>資産の保存先</dt><dd>${scopeName(assetScope)}</dd></div>` : ''}<div><dt>紐づけの管理先</dt><dd>${scopeName(selectedScope)}</dd></div></dl><p>Globalの資産内容は各Projectで共有します。紐づけはProject登録時にコピーされ、その後は自動同期されません。</p></aside>`;
}
function bindingRows(asset, stageId) {
...
function journalSkillPanel(a) {
    const utility = journalSkillKey(a);
    const launchHint = utility === 'journal-review' ? 'Journal Reviewはユーザーが明示的に開始する直接起動Skillです。' : a.useCase ? 'RuntimeからこのSkillを直接使えます。' : '必要なWorkflowから参照して使います。';
    const launch = utility === 'journal' ? `<section class="section"><h3>Journal記録</h3><p class="hint">このSkillは直接起動せず、設定・接続のJournal記録設定でON/OFFを切り替えます。</p></section>` : `<div class="row spread section"><div><h3>直接起動</h3><p class="hint">${launchHint}</p></div><button type="button" role="switch" aria-label="直接起動" aria-checked="${a.useCase}" data-action="usecase:${a.id}" class="${a.useCase ? 'primary' : ''}">${a.useCase ? '有効' : '無効'}</button></div>`;
    const implicitInvocation = a.implicitInvocation === true;
    return `${launch}<div class="row spread section"><div><h3>自動発火</h3><p class="hint">${utility ? '標準のJournal Skillは自動発火しません。' : 'ONにすると、AIが説明をもとにこのSkillを自動で選べます。既定はOFFです。'}</p></div><button type="button" role="switch" aria-label="自動発火" aria-checked="${implicitInvocation}" data-action="implicit-invocation:${a.id}" class="${implicitInvocation ? 'primary' : ''}"${utility ? ' disabled' : ''}>${implicitInvocation ? 'ON' : 'OFF'}</button></div><section class="section"><h3>Runtimeのdescription ${info('Runtimeのdescription', '自動発火がONのときだけ、SkillのdescriptionをRuntime入口へ出力します。')}</h3><p class="body-panel prose">${esc(a.description)}</p></section>`;
}
function implicitInvocationField(a) {
    const utility = a && journalSkillKey(a);
    return `<label class="checkbox-label"><input type="checkbox" name="implicitInvocation"${a?.implicitInvocation ? ' checked' : ''}${utility ? ' disabled' : ''}>自動発火を有効にする</label><p class="hint">${utility ? '標準のJournal Skillは自動発火しません。' : 'ONにすると、AIが説明をもとにこのSkillを自動で選べます。既定はOFFです。'}</p>`;
}
function protectedSkillDetail(a) {
    const relationships = relatedWorkflows(a.id, assets, bindings);
    return `<article class="glass detail"><div class="detail-head"><div><div class="badge-row">${badge(kinds[a.kind])}${badge(labelScope(a.scope))}<span class="mono">rev. ${a.revision}</span></div><h2>${esc(a.name)}</h2><p>${esc(a.explanation || a.description)}</p></div><div class="row">${button(`asset-edit:${a.id}`, '編集する')}</div></div>${scopeSummary(a.scope)}${journalSkillPanel(a)}<section class="section"><h3>本文</h3><div class="body-panel prose">${esc(a.body) || '<span class="muted">未記入</span>'}</div></section>${Object.keys(a.supportingFiles).length ? `<section class="section"><h3>補助ファイル</h3>${Object.entries(a.supportingFiles).map(([f, body]) => `<details><summary>${esc(f)}</summary><pre>${esc(body)}</pre></details>`).join('')}</section>` : ''}<section class="section"><div class="section-header"><h3>参照する資産</h3>${button(`binding-new:${a.id}`, '紐づける', 'small')}</div>${bindingRows(a)}</section><section class="section"><h3>関連するWorkflow / Stage</h3>${relationships.length ? relationships.map(r => `<div class="relation"><div><a href="#assets/${r.workflow.id}">${esc(r.workflow.name)}${r.stageId ? ` / ${esc(r.workflow.stages.find(s => s.id === r.stageId)?.name)}` : ''}</a><small>${r.via.length ? `${esc(r.via.join(' → '))} 経由` : '直接参照'}</small></div></div>`).join('') : '<p class="hint">関連するWorkflowはありません。</p>'}</section><section class="section"><div class="row spread"><span class="mono">${esc(a.id)}</span>${button(`history-asset:${a.id}`, '変更履歴・復元', 'small ghost')}</div><p class="hint">最終更新 ${date(a.updatedAt)}</p></section></article>`;
}
function assetDetail(a) {
    if (journalSkillKey(a))
...
function loadMoreMarkup(page, nextCursor) {
...
function setupRecordObserver(page) {
    recordObserver?.disconnect();
    const sentinel = document.querySelector(`[data-record-load-more="${page}"]`);
    if (!sentinel || sentinel.hidden)
        return;
    recordObserver = new IntersectionObserver(entries => {
        if (entries.some(entry => entry.isIntersecting))
            void loadMoreRecords(page);
    }, { root: document.querySelector('.main-content'), rootMargin: '240px' });
    recordObserver.observe(sentinel);
}
function recordInsightsMarkup(insights, review = false) {
...
function journalRecordMarkup(journal) {
...
function reviewRecordMarkup(item) {
...
function historyRecordMarkup(changeSet) {
...
function journalDetailMarkup(journal, insights) {
...
function reviewDetailMarkup(item, journal, insight) {
    const decisions = ['approved', 'deferred', 'rejected'].filter(decision => decision !== item.lastDecision || item.status === 'pending');
...
function historyDetailMarkup(changeSet, histories, provenance) {
...
async function loadRecordPanel(panel) {
    const kind = panel.dataset.lazyKind, id = panel.dataset.id, generation = recordGeneration;
    if (!kind || !id || panel.dataset.loaded === 'true' || panel.dataset.loading === 'true')
        return;
    panel.dataset.loading = 'true';
    try {
        let html = '';
        if (kind === 'journal') {
            const data = await api('journal.get', { journalId: id });
            html = journalDetailMarkup(data.journal, data.insights);
        }
        else if (kind === 'review') {
            const data = await api('review.item.get', { reviewItemId: id });
            html = reviewDetailMarkup(data.reviewItem, data.journalTask, data.insight);
        }
        else if (kind === 'history') {
            const data = await api('history.get', { changeSetId: id, includeDetails: true });
        ...
        if (generation === recordGeneration && panel.isConnected) {
            panel.querySelector('.record-panel-body').innerHTML = localizeHtml(html, language);
            panel.dataset.loaded = 'true';
        }
    }
    catch (error) {
        if (generation === recordGeneration && panel.isConnected)
            panel.querySelector('.record-panel-body').innerHTML = `<p class="form-error" role="alert">${esc(errorMessage(error))}</p>`;
    }
    finally {
...
async function loadMoreRecords(page) {
    ...
    const button = document.querySelector(`[data-action="record-load:${page}"]`);
    if (button) {
        button.disabled = true;
        button.textContent = localizeHtml('読み込み中…', language);
    }
    try {
        if (page === 'journals') {
            const data = await api('journal.list', { limit: recordPageSize, cursor, ...(selectedScope !== 'global' ? { projectId: selectedScope } : {}) });
            if (generation !== recordGeneration)
                return;
            journalListState.journals.push(...data.journals);
            journalListState.insights.push(...data.insights);
            journalListState.nextCursor = data.nextCursor;
            screenData = { ...screenData, journals: journalListState.journals, insights: journalListState.insights };
            document.querySelector('[data-record-list="journals"]')?.insertAdjacentHTML('beforeend', localizeHtml(data.journals.map(journalRecordMarkup).join(''), language));
        }
        else if (page === 'review') {
            const data = await api('review.pending', { limit: recordPageSize, cursor, include: ['journalTask', 'insights'], includeBodies: false, ...(selectedScope !== 'global' ? { projectId: selectedScope } : {}) });
            if (generation !== recordGeneration)
                return;
            reviewListState.reviewItems.push(...data.reviewItems);
            reviewListState.journals.push(...data.journals);
            reviewListState.insights.push(...data.insights);
            reviewListState.nextCursor = data.nextCursor;
            ...
            document.querySelector('[data-record-list="review"]')?.insertAdjacentHTML('beforeend', localizeHtml(data.reviewItems.map(reviewRecordMarkup).join(''), language));
        }
        else {
            const data = await api('history.get', { limit: recordPageSize, cursor });
            if (generation !== recordGeneration)
                return;
            historyListState.histories.push(...data.histories);
            historyListState.changeSets.push(...data.changeSets);
            historyListState.provenance.push(...data.provenance);
            historyListState.nextCursor = data.nextCursor;
            screenData = { ...screenData, ...historyListState };
            document.querySelector('[data-record-list="history"]')?.insertAdjacentHTML('beforeend', localizeHtml(data.changeSets.map(historyRecordMarkup).join(''), language));
        }
        const next = page === 'journals' ? journalListState.nextCursor : page === 'review' ? reviewListState.nextCursor : historyListState.nextCursor;
        const sentinel = document.querySelector(`[data-record-load-more="${page}"]`);
        if (sentinel) {
            sentinel.hidden = !next;
            sentinel.querySelector('button').textContent = localizeHtml('過去を読み込む', language);
            sentinel.querySelector('.hint').textContent = next ? localizeHtml('スクロールすると過去の記録を読み込みます。', language) : '';
        }
        ...
        const countLabel = document.querySelector('[data-record-count]');
        if (countLabel)
            countLabel.textContent = localizeHtml(`${count}件を表示中`, language);
...
async function render() {
    ...
    resetRecordState();
    try {
        await renderPage(controller.signal);
    }
    catch (error) {
        if (!controller.signal.aborted)
            throw error;
...
async function renderPage(signal) {
    const read = (operation, input = {}) => api(operation, input, false, signal);
    ...
    if (page === 'runs') {
        const data = await read('run.list', selectedScope === 'global' ? {} : { projectId: selectedScope });
        screenData = data;
        const run = data.runs.find(r => r.id === selectedId);
        if (run) {
            const detail = await read('run.inspect', { contextHandle: run.contextHandle });
        ...
        if (page === 'journals') {
            ...
            const [data, proposals] = await Promise.all([
                read('review.pending', { ...(selectedScope !== 'global' ? { projectId: selectedScope } : {}), include: ['journalTask', 'insights'], includeBodies: false, limit: recordPageSize }),
                read('proposal.list', { includeChanges: false }),
            ]);
        ...
        const data = await read('history.get', { limit: recordPageSize });
        ...
        const data = await read('diagnostics.get');
        ...
        const diagnosticAssets = (await Promise.all(Array.from({ length: Math.ceil(missingAssetIds.length / 100) }, (_, index) => read('asset.get_many', { assetIds: missingAssetIds.slice(index * 100, index * 100 + 100), includeDeleted: true })))).flatMap(result => result.assets);
        ...
        const [r, s, candidates] = await Promise.all([read('runtime.list'), read('settings.get'), read('runtime.discover')]);
        const common = selectedScope !== 'global' ? (await read('common.get', { projectId: selectedScope })).common : null;
...
function assetEditor(a, newKind = 'skill') {
    ...
    if (a && journalSkillKey(a)) {
        ...
        modal('資産を編集', `<form data-form="asset" data-id="${a.id}" data-kind="skill" class="form-stack"><div class="grid-two"><label><span class="field-label">名前</span><input aria-label="名前" name="name" type="text" value="${esc(a.name)}" readonly required></label><input type="hidden" name="scope" value="${esc(a.scope)}"></div>${area('コメント', 'explanation', a.explanation || a.description, true, false, 'AACL内でこのSkillを見分けるための短い補足です。Runtime YAMLのdescriptionとは別に管理します.')}${field('説明（Runtime YAML）', 'description', a.description, false, 'text', '自動発火がONのときだけ、SkillのdescriptionをRuntime入口へ出力します。')}${area('本文（Markdown）', 'body', a.body, true, true)}${useCase}${implicitInvocationField(a)}<section><div class="section-header"><h3>補助ファイル</h3>${button('file-add', '＋ 追加', 'small')}</div><div id="file-rows">${Object.entries(a.supportingFiles).map(([path, body]) => fileRow(path, body)).join('')}</div></section><p class="hint">JournalとJournal Reviewの標準Skillは、名前と削除状態を変更できません。</p>${formEnd()}</form>`);
        return;
    }
...
function readTransitions(form) { return [...form.querySelectorAll('.transition-row')].map(row => { const from = row.closest('.stage-editor')?.dataset.id; if (!from)
    throw new Error('遷移元の工程を確認できません。'); return { id: row.dataset.id, from, to: row.querySelector('[name=to]').value, condition: row.querySelector('[name=transitionCondition]').value, label: row.querySelector('[name=transitionLabel]').value }; }); }
function updateTransitionTargets(form) {
    const stages = readStages(form);
    for (const target of form.querySelectorAll('.transition-row [name=to]')) {
...
function updateStageNumbers(form) { form.querySelectorAll('.stage-editor .stage-title').forEach((label, index) => { label.textContent = localizeHtml(`工程 ${index + 1}`, language); }); }
function updateTransitionNumbers(stage) { stage.querySelectorAll('.transition-row').forEach((row, index) => { const label = `遷移設定 ${index + 1}`; row.querySelector('legend').textContent = localizeHtml(label, language); row.querySelector('.transition-remove').setAttribute('aria-label', localizeHtml(`${label}を削除`, language)); }); }
function updateTransitionEmptyState(stage) {
    const list = stage.querySelector('.stage-transition-list');
    if (!list.querySelector('.transition-row'))
        list.innerHTML = localizeHtml('<p class="hint stage-transition-empty">行き先はまだありません。</p>', language);
}
function bindingEditor(sourceId, stageId, existing) {
...
function journalEditor(run) {
    const template = '## Task\n\n## 実際に使ったもの\ntools: []\nskills: []\nrules: []\n\n## 良かった点\n\n## 困った点\n\n## 改善の種\n\n## 根拠・確かさ\n\n## 日付\n\n## Project\n\n## Branch\n\n## Type\n';
    modal('Journalを記録', `<form data-form="journal" data-run="${run?.id ?? ''}" class="form-stack">${field('Task（作業名）', 'task', '', !run)}${area('Journal（Markdown）', 'body', template, true, true)}<p class="hint">Tool・Skill・Ruleを分けて記録できます。書くことのない項目は省略し、気づきは空行で区切ると個別に扱えます。</p>${formEnd('Journalを保存')}</form>`);
}
async function action(value, target) {
    ...
    if (key === 'asset-new' || key === 'new-kind') {
        assetEditor(undefined, id || 'skill');
        return;
    }
    if (key === 'asset-edit') {
        if (!a)
    ...
    if (key === 'asset-delete') {
        const preview = await api('asset.delete.preview', { assetId: id });
    ...
    if (key === 'role-create') {
        const row = target.closest('.stage-editor'), panel = row.querySelector('.new-role-fields'), role = row.querySelector('[name=stageRole]');
        panel.hidden = !panel.hidden;
        role.disabled = !panel.hidden;
        target.textContent = localizeHtml(panel.hidden ? '＋ 新しいRole' : '作成をやめる', language);
        if (!panel.hidden) {
            panel.dataset.previousRole = role.value;
            role.value = '';
            row.querySelector('[name=newRoleName]')?.focus();
        }
        else
            role.value = panel.dataset.previousRole ?? '';
        return;
    }
    if (key === 'choice-add') {
    ...
    if (key === 'choice-condition-add') {
        const model = assets.find(asset => asset.id === target.closest('form')?.dataset.source && asset.kind === 'model');
        const rows = dialog.querySelectorAll('.choice-condition-row').length;
        if (model)
            dialog.querySelector('#choice-condition-rows').insertAdjacentHTML('beforeend', localizeHtml(modelChoiceConditionRow(model, {}, rows), language));
        return;
    }
    ...
    if (key === 'choice-option-remove') {
        const row = target.closest('.model-choice-editor-row');
        const options = row.querySelectorAll('.model-option-row');
        if (options.length <= 1)
            throw new Error('選択肢には少なくとも1つの値が必要です。');
        target.closest('.model-option-row')?.remove();
        return;
    }
    if (key === 'file-add') {
        dialog.querySelector('#file-rows').insertAdjacentHTML('beforeend', localizeHtml(fileRow(), language));
        return;
    }
    if (key === 'transition-add') {
        const stage = target.closest('.stage-editor'), stages = readStages(dialog);
        if (!stage || !stages.length)
            throw new Error('先に工程を追加してください。');
        ...
        stage.querySelector('.stage-transition-list').insertAdjacentHTML('beforeend', localizeHtml(transitionRow(transition, stages, stage.querySelectorAll('.transition-row').length), language));
        updateTransitionNumbers(stage);
        return;
    }
    if (key === 'row-remove') {
        const transition = target.closest('.transition-row');
        if (transition) {
            const stage = transition.closest('.stage-editor');
            transition.remove();
            updateTransitionNumbers(stage);
            updateTransitionEmptyState(stage);
            return;
        }
        const stage = target.closest('.stage-editor');
        if (stage) {
            const stageId = stage.dataset.id;
            for (const row of dialog.querySelectorAll('.transition-row'))
                if (row.closest('.stage-editor') === stage || row.querySelector('[name=to]').value === stageId)
                    row.remove();
            stage.remove();
            updateStageNumbers(dialog);
            updateTransitionTargets(dialog);
            dialog.querySelectorAll('.stage-editor').forEach(stage => { updateTransitionNumbers(stage); updateTransitionEmptyState(stage); });
            return;
        }
    ...
    if (key === 'usecase') {
        await api('skill.usecase', { assetId: id, enabled: !a.useCase, provenance: provenance('直接起動設定を変更') }, true);
        await refresh();
        return;
    }
    if (key === 'implicit-invocation') {
        await api('asset.update', { id, expectedRevision: a.revision, asset: { implicitInvocation: !a.implicitInvocation }, provenance: provenance('自動発火設定を変更') }, true);
        await refresh();
        return;
    }
    if (key === 'binding-new') {
        bindingEditor(id, extra);
        return;
    }
    if (key === 'binding-edit') {
        const b = bindings.find(b => b.id === id);
        bindingEditor(b.sourceId, b.stageId, b);
        return;
    }
    if (key === 'binding-remove') {
        const binding = bindings.find(value => value.id === id);
        if (!binding)
            throw new Error('紐づけを再取得してください。');
        await api('binding.remove', { id, expectedRevision: binding.revision, provenance: provenance('紐づけを解除') }, true);
        notify('紐づけを解除しました。');
        await refresh();
        return;
    }
    if (key === 'run-new') {
        ...
        if (!workflows.length) {
            notify('先にWorkflowを作成してください。');
            assetEditor(undefined, 'workflow');
            return;
        }
        modal('Workflow Runを開始', `<form data-form="run" class="form-stack">${assetSelect('Workflow', 'workflowId', workflows.map(a => opt(a.id, a.name, id)).join())}${select('実行するRuntime', 'runtime', opt('claude', 'Claude Code') + opt('codex', 'Codex'))}${area('実行する依頼', 'instruction')}${field('対象', 'target', '', false)}${scopeSummary()}<p class="hint">選択中の管理先の紐づけで実行します。</p>${formEnd('Runを開始')}</form>`);
        return;
    }
    if (key === 'run-transition' || key === 'run-cancel') {
        const selectedTransition = key === 'run-transition' ? (screenData.detail?.snapshot?.workflow.transitions.find(t => t.id === extra) ?? null) : null;
        modal(key === 'run-cancel' ? 'Runを中止' : '工程を進める', `<form data-form="${key}" data-run="${id}" data-transition="${esc(extra ?? '')}" class="form-stack">${selectedTransition ? `<p class="hint">遷移条件: ${esc(selectedTransition.condition)}</p>` : ''}${area(key === 'run-cancel' ? '中止理由' : '遷移判断の報告', 'report')}${formEnd(key === 'run-cancel' ? '中止する' : '選択した遷移を実行')}</form>`);
        return;
    }
    if (key === 'journal-new') {
        const settings = await api('settings.get');
        if (!settings.journalEnabled) {
            notify('Journal記録が無効です。設定・接続で有効にしてください。');
            return;
        }
        const run = screenData.runs?.find(r => r.id === id);
        journalEditor(run);
        return;
    }
    if (key === 'insight') {
        await api('insight.status', { insightId: id, status: extra }, true);
        notify('気づきの状態を更新しました。');
        await refresh();
        return;
    }
    if (key === 'review-decide') {
        modal('Review項目への判断', `<form data-form="review-decision" data-id="${id}" data-decision="${extra}" class="form-stack"><p>判断: ${states[extra]}</p>${area('判断の内容', 'note', '', false)}${formEnd('判断を記録')}</form>`);
        return;
    }
    if (key === 'proposal-new') {
        const journals = screenData.journals ?? [], summaries = screenData.insights ?? [];
        if (!journals.length)
            throw new Error('提案の根拠となるJournalを先に記録してください。');
        const pending = summaries.filter(insight => insight.status === 'pending');
        const details = await Promise.all([...new Set(pending.map(insight => insight.journalId))].map(journalId => api('journal.get', { journalId })));
        const insights = details.flatMap(detail => detail.insights).filter(insight => pending.some(summary => summary.id === insight.id));
        modal('改善を提案', `<form data-form="proposal" class="form-stack">${field('提案名', 'title')}${area('観測した状況', 'observedContext')}${area('変更の内容', 'proposedChange')}${area('理由', 'reason')}${assetSelect('変更する資産', 'assetId', assets.map(a => opt(a.id, `${kinds[a.kind]} / ${a.name}`)).join())}${area('更新後の本文・責務', 'body', '', true, true)}<fieldset><h3>根拠Journal・レビュー対象</h3>${journals.map(j => `<label class="checkbox-label"><input type="checkbox" name="journalIds" value="${j.id}">${esc(j.task || date(j.createdAt))}</label>`).join('')}</fieldset><fieldset><h3>適用時に処理する気づき</h3>${insights.filter(i => i.status === 'pending').map(i => `<label class="checkbox-label"><input type="checkbox" name="insightIds" value="${i.id}">${esc(`${journals.find(journal => journal.id === i.journalId)?.task || 'Journal'} / ${i.body.trim().slice(0, 90)}`)}</label>`).join('')}</fieldset><p class="hint">Workflow構成や複数資産の変更を含む提案は、接続中のAIから作成できます。</p>${formEnd('提案を保存')}</form>`);
        return;
    }
    if (key === 'proposal-decide') {
        modal('提案への判断', `<form data-form="decision" data-id="${id}" data-choice="${extra}" class="form-stack"><p>判断: ${states[extra]}</p>${area('判断の内容', 'note')}${formEnd('判断を記録')}</form>`);
        return;
    }
    if (key === 'proposal-apply') {
        await api('proposal.apply', { proposalId: id }, true);
        notify('変更を適用し、対象の気づきを処理済みにしました。');
        await refresh();
        return;
    }
    if (key === 'history-asset') {
        const data = await api('history.get', { entityId: id });
        const current = data.revisions[0];
        modal('資産の履歴と復元', `<p>${esc(current.name)} · 現在 rev. ${current.revision}</p><div class="stack">${data.revisions.map(r => `<div class="editor-row"><div class="row spread"><strong>rev. ${r.revision}</strong><span class="mono">${date(r.updatedAt)}</span>${r.revision !== current.revision ? button(`asset-restore:${id}:${r.revision}`, 'この版を復元', 'small') : badge('現在')}</div><div class="diff"><div><small>この版</small><pre>${esc(JSON.stringify(r, null, 2))}</pre></div><div><small>現在</small><pre>${esc(JSON.stringify(current, null, 2))}</pre></div></div></div>`).join('')}</div>`);
        return;
    }
    if (key === 'asset-restore') {
        await api('asset.restore', { assetId: id, revision: Number(extra), expectedRevision: a.revision }, true);
        dialog.close();
        notify('選択した版を新しいrevisionとして復元しました。');
        await refresh();
        return;
    }
    if (key === 'changeset-restore') {
        const loaded = await api('history.get', { changeSetId: id, includeDetails: true });
        const c = loaded.changeSets[0];
        if (!c)
            throw new Error('変更履歴が見つかりません。');
        modal('変更前の状態へ復元', `<p>対象の変更前の内容を、新しいrevisionとして保存します。</p>${details('対象の変更', c)}<form data-form="changeset-restore" data-id="${id}">${formEnd('復元する')}</form>`);
        return;
    }
    if (key === 'project-new') {
        modal('Projectを登録', `<form data-form="project" class="form-stack">${field('Project名', 'name')}${field('Project root（絶対パス）', 'root')}<p class="hint">登録時点のGlobalの紐づけをコピーします。その後のGlobalの変更は自動同期されません。</p>${formEnd('Projectを登録')}</form>`);
        return;
    }
    if (key === 'common-edit') {
    ...
    if (key === 'runtime-new') {
        modal('Runtime設定先を追加', `<form data-form="runtime" class="form-stack">${select('Runtime', 'runtime', opt('claude', 'Claude Code') + opt('codex', 'Codex'))}${select('実行環境', 'platform', opt('wsl', 'WSL') + opt('windows', 'Windows'))}${field('設定先（.claude / .codex の絶対パス）', 'path')}<p class="hint">検出した標準設定先:</p><pre>${esc(screenData.candidates.map(c => c.path).join('\n'))}</pre><p class="hint">管理先: ${esc(labelScope(selectedScope))}</p>${formEnd('追加して入口を生成')}</form>`);
        return;
    }
    if (key === 'journal-skills') {
        await api('setup.skills', {}, true);
        notify('JournalとJournal ReviewのSkillを導入しました。');
        await refresh();
        return;
    }
    if (key === 'runtime-sync') {
        await api('runtime.sync', {}, true);
        notify('入口を同期しました。');
        await refresh();
        return;
    }
    if (key === 'runtime-remove') {
        await api('runtime.unregister', { targetId: id }, true);
        notify('管理を解除しました。生成済みファイルは残っています。');
        await refresh();
        return;
    }
    if (key === 'backup' || key === 'export') {
        modal(key === 'backup' ? 'Backupを保存' : 'データを出力', `<form data-form="${key}" class="form-stack">${field(key === 'backup' ? '新規出力fileの絶対パス' : '新規出力directoryの絶対パス', 'path')}${formEnd('指定先へ保存')}</form>`);
    }
}
async function submit(form) {
    const data = new FormData(form), get = (key) => String(data.get(key) ?? ''), key = form.dataset.form;
    if (key === 'asset-delete') {
        if (!data.has('confirm'))
            throw new Error('削除を確認してください。');
        ...
        if (new Set(files.map(([name]) => name)).size !== files.length)
            throw new Error('補助ファイル名が重複しています。');
        const choices = kind === 'model' ? readModelChoices(form) : [];
        if (kind === 'model' && choices.some(choice => !choice.name.trim() || choice.options.some(option => !option.trim())))
            throw new Error('Modelの選択肢名と選択値を入力してください。');
        ...
        const asset = { kind, name: get('name'), description, explanation, scope: get('scope'), modelName: kind === 'model' ? get('modelName') : old?.modelName ?? '', invocationMethod: kind === 'model' ? get('invocationMethod') : old?.invocationMethod ?? '', choices, body: kind === 'role' || kind === 'workflow' || kind === 'model' ? old?.body ?? '' : get('body'), responsibilities: kind === 'role' ? get('body') : old?.responsibilities ?? '', useCase: kind === 'skill' && data.has('useCase'), implicitInvocation: kind === 'skill' && data.has('implicitInvocation'), supportingFiles: Object.fromEntries(files), stages, transitions, entryStage: old?.entryStage && stages.some(s => s.id === old.entryStage) ? old.entryStage : stages[0]?.id ?? '', metadata: old?.metadata ?? {} };
        if (kind === 'workflow') {
            const workflowId = old?.id ?? crypto.randomUUID(), newRoleChanges = [];
            const assignments = [...form.querySelectorAll('.stage-editor')].map(row => {
                const stageId = row.dataset.id, panel = row.querySelector('.new-role-fields');
                if (!panel.hidden) {
                    const roleName = row.querySelector('[name=newRoleName]').value.trim();
                    const description = row.querySelector('[name=newRoleDescription]').value.trim();
            ...
            const workflowChange = old ? { type: 'asset.save', id: workflowId, expectedRevision: old.revision, asset } : { type: 'asset.create', id: workflowId, asset };
            const changes = [
        ...
        const run = screenData.runs.find(r => r.id === form.dataset.run);
        const result = await api(key === 'run-cancel' ? 'run.cancel' : 'run.transition', key === 'run-cancel' ? { contextHandle: run.contextHandle, reason: get('report') } : { contextHandle: run.contextHandle, version: run.version, transitionId: form.dataset.transition, report: get('report') }, true);
        if (result.outcome === 'stale')
            throw new Error('Runは別の操作で進んでいます。一覧を再読み込みしてください。');
    }
    else if (key === 'journal') {
        const run = screenData.runs?.find(r => r.id === form.dataset.run);
    ...
    else if (key === 'review-decision')
        await api('review.decide', { reviewItemId: form.dataset.id, decision: form.dataset.decision, note: get('note') }, true);
    else if (key === 'decision')
        await api('proposal.decide', { proposalId: form.dataset.id, choice: form.dataset.choice, note: get('note') }, true);
    else if (key === 'changeset-restore')
        await api('changeset.restore', { changeSetId: form.dataset.id }, true);
    else if (key === 'project')
        await api('project.init', { name: get('name'), root: get('root') }, true);
    else if (key === 'common')
        await api('common.save', { projectId: selectedScope, expectedRevision: screenData.common.revision, ruleIds: data.getAll('ruleIds'), provenance: provenance('Project共通Ruleを編集') }, true);
    else if (key === 'runtime')
        await api('runtime.register', { runtime: get('runtime'), platform: get('platform'), path: get('path'), scope: selectedScope }, true);
    else if (key === 'settings')
        await api('settings.save', { timeoutHours: Number(get('timeoutHours')), journalEnabled: data.has('journalEnabled') }, true);
    else if (key === 'export' || key === 'backup')
...
document.addEventListener('toggle', event => {
    ...
    if (panel?.open && panel.dataset.lazyKind)
...
document.addEventListener('submit', event => {
    ...
    const button = form.querySelector('button[type=submit]'), error = form.querySelector('.form-error');
    button.disabled = true;
    error.textContent = '';
...
document.addEventListener('change', event => {
    ...
    if (input.id === 'scope-select') {
        selectedScope = input.value;
        location.hash = route()[0];
        void refresh();
    }
    ...
    if (input.name === 'stageModel') {
        const row = input.closest('.stage-editor');
        if (row)
            row.querySelector('.model-choice-container').innerHTML = localizeHtml(modelChoiceFields(input.value), language);
    }
    if (input.name === 'targetId' || input.name === 'purpose') {
        const form = input.closest('[data-form=binding]');
        const target = form ? assets.find(asset => asset.id === form.querySelector('[name=targetId]')?.value) : undefined;
        const purpose = form?.querySelector('[name=purpose]')?.value;
        if (form)
            form.querySelector('.binding-model-choice-fields').innerHTML = target?.kind === 'model' && purpose === 'stage-model' ? localizeHtml(modelChoiceFields(target.id), language) : '';
    }
});
