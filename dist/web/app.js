...
function transitionRow(t, stages, index) {
    return `<fieldset class="transition-row" data-id="${esc(t.id)}"><legend>遷移設定 ${index + 1}</legend><div class="transition-fields">${select('行き先', 'to', stages.map(s => opt(s.id, s.name || '未命名の工程', t.to)).join('') + opt('completed', '完了', t.to))}${area('遷移条件', 'transitionCondition', t.condition)}${field('表示名', 'transitionLabel', t.label)}<button type="button" class="ghost transition-remove" data-action="row-remove" aria-label="遷移設定 ${index + 1}を削除">×</button></div></fieldset>`;
}
