...
function autoInvocationPanel(a) {
    return `<div class="row spread section"><div><h3>自動発火</h3><p class="hint">${a.autoInvocation ? 'descriptionに一致したときRuntimeから自動発火できます。' : 'Runtimeから自動発火しません。'}</p></div><button type="button" role="switch" aria-checked="${a.autoInvocation}" data-action="autoinvocation:${a.id}" class="${a.autoInvocation ? 'primary' : ''}">${a.autoInvocation ? '有効' : '無効'}</button></div>`;
}
function journalSkillPanel(a) {
    const utility = journalSkillKey(a);
    const direct = utility === 'journal'
        ? `<section class="section"><h3>Journal記録</h3><p class="hint">このSkillは直接起動せず、設定・接続のJournal記録設定でON/OFFを切り替えます。</p></section>`
        : `<div class="row spread section"><div><h3>直接起動</h3><p class="hint">${utility === 'journal-review' ? 'Journal Reviewはユーザーが明示的に開始する直接起動Skillです。' : a.useCase ? 'RuntimeからこのSkillを直接使えます。' : '必要なWorkflowから参照して使います。'}</p></div><button type="button" role="switch" aria-checked="${a.useCase}" data-action="usecase:${a.id}" class="${a.useCase ? 'primary' : ''}">${a.useCase ? '有効' : '無効'}</button></div>`;
    return `${direct}${autoInvocationPanel(a)}<section class="section"><h3>Runtimeのdescription ${info('Runtimeのdescription', 'Runtime YAMLへ出力されるSkillの説明です。')}</h3><p class="body-panel prose">${esc(a.description)}</p></section>`;
}
...
function assetDrawer(a) {
    return `<div class="asset-drawer-backdrop" data-action="asset-close" aria-hidden="true"></div><aside class="asset-drawer" aria-label="${esc(a.name)}の詳細"><div class="asset-drawer-head"><span>資産の詳細</span>${button('asset-close', '×', 'icon-button')}</div>${assetDetail(a)}${a.kind === 'skill' && !journalSkillKey(a) ? autoInvocationPanel(a) : ''}</aside>`;
}
...
async function action(value, target) {
    ...
    if (key === 'autoinvocation') {
        await api('skill.autoinvocation', { assetId: id, enabled: !a.autoInvocation, provenance: provenance('自動発火設定を変更') }, true);
        await refresh();
        return;
    }
    if (key === 'binding-new') {
        ...
        if (kind === 'workflow') {
            ...
            const assignments = [...form.querySelectorAll('.stage-editor')].map(row => {
                ...
                if (!panel.hidden) {
                    ...
                    newRoleChanges.push({ type: 'asset.create', id: roleId, asset: { kind: 'role', name: roleName, description, explanation: '', body: '', responsibilities: row.querySelector('[name=newRoleResponsibilities]').value, scope: 'global', useCase: false, autoInvocation: false, modelName: '', invocationMethod: '', choices: [], metadata: {}, supportingFiles: {}, stages: [], transitions: [], entryStage: '' } });
