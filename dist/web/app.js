...
async function action(value, target) {
    ...
    if (key === 'project-new') {
        modal('Projectを登録', `<form data-form="project" class="form-stack">${field('Project名', 'name')}${field('Project root（絶対パス）', 'root')}<p class="hint">Globalの紐づけをコピーし、Claude Code / Codexの入口を作成します。</p>${formEnd('Projectを登録')}</form>`);
        return;
    }
    if (key === 'common-edit') {
