...
export class Core {
    ...
    deleteAsset(input, provenance) {
        ...
        const changes = preview.bindings.map(b => ({ type: 'binding.remove', id: b.id }));
    ...
    validateBinding(b) {
        ...
        if (b.purpose !== 'reference' && (source.kind !== 'workflow' || target.kind !== 'role'))
            throw new Error('担当RoleはWorkflowまたはStageに指定してください。');
        if (b.purpose === 'entry-role' && b.stageId)
            throw new Error('entry roleはWorkflowに指定してください。');
        if (b.purpose === 'stage-role' && !b.stageId)
            throw new Error('担当Stageを指定してください。');
    ...
    resolve(snapshot, stageId, roleId) {
    ...
    startRun(input) {
        ...
        const run = this.store.put('run', { id: runId, contextHandle, workflowId: workflow.id, workflowRevision: workflow.revision, projectId: project?.id, snapshotId,
            ...
            taskType: workflow.taskType, lastActivity: new Date().toISOString(), }, scope);
    ...
    transition(input) {
        ...
        const result = this.store.put('run', { ...run, status: transition.to === 'completed' ? 'completed' : 'active',
            stageId: transition.to === 'completed' ? run.stageId : transition.to, version: run.version + 1, }, run.projectId ?? 'global');
    ...
    review() {
        const insights = this.store.list('insight').filter(i => i.status === 'pending');
        const reviewed = new Set(this.store.list('proposal').flatMap(p => p.reviewedJournalIds));
        const journals = this.store.list('journal').filter(j => !reviewed.has(j.id) || insights.some(i => i.journalId === j.id));
    ...
    applyProposal(proposalId) {
        ...
        for (const id of p.insightIds)
            this.store.put('insight', { ...this.store.get(id, 'insight'), status: 'processed' });
        return result;
    }
    restoreAsset(assetId, revision) {
    ...
    restoreChangeSet(id) {
        const cs = this.store.get(id, 'changeset'), operations = [], restored = [];
        for (const historyId of [...cs.historyIds].reverse()) {
            const h = this.store.get(historyId, 'history');
            if (h.before === null) {
                if (h.kind === 'binding')
                    operations.push({ type: 'binding.remove', id: h.entityId });
                if (h.kind === 'asset') {
                    const a = this.asset(h.entityId);
                    operations.push({ type: 'asset.save', id: a.id, asset: { ...this.assetPayload(a), body: '処置なし', ...(a.kind === 'role' ? { responsibilities: '処置なし' } : {}) } });
                }
            }
            else {
                restored.push({ entityId: h.entityId, revision: h.before });
                if (h.kind === 'asset')
                    operations.push({ type: 'asset.save', id: h.entityId, asset: this.assetPayload(this.store.revision(h.entityId, h.before)) });
                if (h.kind === 'binding') {
                ...
                if (h.kind === 'common') {
                    ...
                    operations.push({ type: 'common.save', projectId: c.projectId, ruleIds: c.ruleIds });
                }
            }
        }
