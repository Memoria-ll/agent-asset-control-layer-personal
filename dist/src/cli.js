...
import { serve, errorMessage } from './server.js';
...
if (!Number.isInteger(port) || port < 1 || port > 65535)
    throw new Error('portは1〜65535で指定してください。');
const command = positionals[0] ?? 'help';
async function health() {
    try {
        const response = await fetch(`${url}/health`, { signal: AbortSignal.timeout(700) });
        if (!response.ok)
            throw new Error('portを別のServiceが使用しています。');
        const result = await response.json();
        if (result.service !== 'aacl' || result.dataDirectory !== directory)
            throw new Error('このportは別のServiceまたは別のAACL管理フォルダーで使用中です。');
        return result;
    }
    catch (error) {
...
function print(data) { process.stdout.write(`${typeof data === 'string' ? data : JSON.stringify(data, null, 2)}\n`); }
function installApplication(target, replace = false) {
...
async function main() {
    if (command === 'serve') {
        ...
        print(await api('project.init', { root: process.cwd(), name: basename(process.cwd()), operationId: randomUUID() }));
    else if (command === 'diagnostics')
        print(await api('diagnostics.get'));
    else if (command === 'export') {
        if (!positionals[1])
            throw new Error('出力先directoryを指定してください。');
        print(await api('data.export', { directory: resolve(positionals[1]) }));
    }
    else if (command === 'backup') {
        if (!positionals[1])
            throw new Error('出力先fileを指定してください。');
        print(await api('data.backup', { path: resolve(positionals[1]) }));
    }
    else if (command === 'restore') {
        if (!positionals[1] || !values.dir)
            throw new Error('Backup fileと、新規復元先の--dirを指定してください。');
        ...
        if (action === 'enable') {
            if (!existsSync(join(directory, '.aacl-managed')) || !existsSync(join(directory, 'bin/aacl')))
                throw new Error('先にaacl setupを実行してください。');
            print(`Windowsログオン時に自動起動します: ${autostart.enable()}`);
        }
        else if (action === 'disable') {
            autostart.disable();
            print('Windowsログオン時の自動起動を解除しました。');
        }
        else if (action === 'status') {
            print(autostart.available ? `Windowsログオン時の自動起動: ${autostart.status() ? '有効' : '無効'}` : 'Windowsログオン時の自動起動: WSL外では確認できません');
        }
        else
            throw new Error('使い方: aacl autostart enable|disable|status');
    }
        ...
        if (await health())
            print(await api('service.stop'));
        else
            print('Serviceは停止しています。');
    }
    else if (command === 'uninstall') {
        if (!values.yes)
            throw new Error(`削除範囲: ${directory}\n確認後に--yesを付けて実行してください。Runtime入口は残ります。`);
        ...
        if (new WindowsAutostart(directory).available)
            new WindowsAutostart(directory).disable();
        if (await health()) {
            await api('service.stop');
            for (let i = 0; i < 40 && await health(); i++)
                await delay(100);
            if (await health())
                throw new Error('Serviceを停止できません。');
        }
