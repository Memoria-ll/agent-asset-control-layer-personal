...
import { RuntimeEntries, workflowExecutionInstructions } from './runtime.js';
import { installJournalSkills } from './journal-skills.js';
import { backupData, exportData } from './maintenance.js';
import { assetPatchSchema, assetSchema, bindingSchema, changeSchema, id, journalTemplate, provenanceSchema, revision, scope, text } from './schema.js';
...
export class Operations {
    ...
    constructor(core) {
        ...
        read('proposal.list', '改善提案・ユーザー判断・適用結果を一覧', { includeChanges: z.boolean().default(false) }, p => ({ proposals: store.list('proposal').map(proposal => {
                ...
                return summary;
            }), decisions: store.list('decision'), changeSets: store.list('changeset').filter(c => c.proposalId).map(changeSet => {
                if (p.includeChanges)
                    return changeSet;
        ...
        read('runtime.list', 'Runtime設定先と入口・補助ファイルの状態を確認', {}, () => ({ targets: store.list('runtime-target'), entries: store.list('runtime-entry'), files: store.list('runtime-file') }));
    ...
    register(name, description, schema, handler, write = false, sync = false, mcpVisible = true) {
