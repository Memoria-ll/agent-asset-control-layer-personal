...
Assetの管理先に対応するGlobalまたはProject scopeへ、Claude CodeまたはCodexのRuntime設定先を登録します。Workflow、直接起動または自動発火が有効なSkillの入口が生成されます。`journal`の入口は初期状態では生成されません。自動発火が有効なSkillはCanonical Skillの`description`をそのままCodexのYAMLへ出力し、紐づけで参照されてもContextのSkill候補へ重ねて渡しません。
...
Run開始時に、選択したWorkflow、選択肢条件に一致した関連Asset、Binding、Project Common設定を不変Snapshotへ固定します。初期Contextには現在の工程、担当Role、指定Modelの選択肢展開済みModel名・呼び出し方・選択値、適用するRule本文、候補Skillの説明、サブエージェント継続指示を含めます。自動発火が有効なSkillはRuntimeが既に認識しているため、紐づけが一致してもSkill候補とloaderへ再度渡しません。その他のSkillはホストがAACL Asset ID・revisionのloader対応を内部で保持し、選択された本文を`aacl_skill_get`で取得します。同名のローカルSkillへ推測フォールバックしません。Skill本文と補助ファイルは必要時にSnapshotの固定revisionから取得します。
