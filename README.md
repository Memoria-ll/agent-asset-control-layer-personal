...
Register a Claude Code or Codex Runtime target for the Global or Project scope where its Assets belong. AACL generates entries for Workflows and Skills enabled for direct or automatic invocation. An automatically invocable Skill keeps its canonical `description` in the Codex YAML and is not repeated in Context candidates when a binding reaches it:
...
The entry refers to the canonical Asset by ID and retrieves its instructions from AACL. The body stays in SQLite. On Windows targets, the generated entry invokes AACL through `wsl.exe`. Codex stores the automatic-invocation switch in `agents/openai.yaml` as `policy.allow_implicit_invocation`. If a managed entry has been changed after generation, synchronization records a diagnostic instead of replacing that content. Unregistering a Runtime target leaves its generated entries in place.
...
At Run start, AACL pins the selected Workflow, choice-matching Assets, bindings, and Project Common settings in an immutable Snapshot. The initial Context includes the current stage, its Role, selected Model choices, the resolved Model name and invocation method, applicable Rule bodies, and candidate Skill names plus Runtime descriptions. Automatically invocable Skills are already known by the Runtime, so a matching binding does not repeat them in the Context catalog or loader mapping. Other Skills keep their AACL asset/revision loader mapping and are retrieved through `aacl_skill_get`; the host does not resolve a same-named local Skill as a fallback. Skill bodies and supporting files are retrieved from the Snapshot's pinned revisions when needed.
